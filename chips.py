...
def render_chips_png(rows: List[Dict[str, Any]], save_path: str, stock_id: str = "") -> str:
    ...
    from wayne_navigator import CARD_PNG_DPI, _CARD, _fp, _text_w, _telegram_fig_dpi, mpl_render
    ...
    with mpl_render():
        ...
        fig_w = 7.2
        fig_h_in = H * 0.078
        chips_dpi = _telegram_fig_dpi(fig_w, fig_h_in, CARD_PNG_DPI)
        fig, ax = plt.subplots(figsize=(fig_w, fig_h_in), dpi=chips_dpi, facecolor=C["page"])
        ax.set_xlim(0, 100)
        ax.set_ylim(0, H)
        ax.axis("off")
        ...
        fig.savefig(save_path, dpi=chips_dpi, facecolor=fig.get_facecolor())
        plt.close(fig)
    return save_path
