...
def window_vs_bars(
    bars: Sequence[Dict[str, Any]], *, skip_vol: bool = False
) -> str:
    """近窗官方柱＝這段時間量價照片。不另存 PNG。"""
    if len(bars) < 8:
        return ""
    first = bars[0]
    last = bars[-1]
    ...
    for b in bars:
        try:
            ...
            continue
        if lv > 0 and (low_v <= 0 or lv < low_v):
            low_v = lv
    ...
    bits = [
        f"近窗{first.get('date')}–{last.get('date')}"
        f" 近高{gate_d} {_px(gate_h)} 近低{low_d} {_px(low_v)}"
    ]
    if avg and not skip_vol:
        bits.append(f"均量{_px(avg)}")
    bits.append(
        f"這根高{_px(last.get('high'))}低{_px(last.get('low'))}"
        f"收{_px(last.get('close'))}量"
        f"{int(last_v) if last_v is not None and not skip_vol else '—'}"
    )
    if kind_zh:
        bits.append(kind_zh)
    return "；".join(bits)
...
def structure_vs_spoken(
    ...
    """用官方高低量重述他為何能這樣說。點名檔必寫近窗，不准等提醒。不准發明 5／9。不是買訊。"""
    blob = spoken or ""
    win = window_vs_bars(bars, skip_vol=skip_vol)
    bits: List[str] = []
    if win:
        bits.append(win)
    if len(bars) >= 8:
