# -*- coding: utf-8 -*-
"""哥哥圖文說明：手機長圖 6 頁，說明頁「圖文」一次一張、按第 N 張換頁。
...
PAGES: Sequence[Tuple[str, str, str]] = (
    (
        "cover",
        "第一次用，先做這三步",
        "這本說明給手機看。一次只看一張。現在共 6 張。按「第 2 張」看下一張。\n"
        "\n"
        "1  點四格鍵盤圖示叫出兩排。不見就打 /menu\n"
        "2  直接打代號看圖，例如 2330、0050、0052、00631L、00981A。不要先按「刷新上一檔」\n"
    ...
    (
        ...
        "\n"
        "第一排（左到右）\n"
        "說明　海選　持股　觀察　刷新上一檔　回報\n"
        "\n"
        "第二排（左到右）\n"
        "大盤　資金　當沖　隔日沖　AI倉　連買區\n"
        "\n"
        "刷新上一檔＝只刷新上一檔決策卡，不是海選。打「決策卡」也行。還沒查過請直接打代號（股票或 ETF）。\n"
        "打「精簡選單」只留這六顆（兩排各三）；「完整選單」恢復十二顆。畫面怪按第一排最右「回報」。",
    ),
    (
        "lookup",
        "查一檔：一次三張圖",
        "打股名或代號，例如 2330、0050、0052、00631L、00981A。不要先按「刷新上一檔」。\n"
        "\n"
        "1  輸入列打代號或股名，送出就出圖。撞名或國字打不準會列出相近的，點左邊確認才出圖。KY 可只打前面。ETF 可含 L／R／A；海選名單仍只有股票／KY\n"
        "2  一次出三張：介紹圖（熱不熱、獲利離 0）／決策卡（高低卡表，進場只認表）／導航圖（近半年）\n"
        "3  圖下面那一排：籌碼、營收、產業（一張圖卡，股名旁公開細項小框，沒抓到不畫）、觀察、記買入。零股寫「200股 631.6」\n"
        "\n"
        "黃金買點＝獲利剛離開 0。重點觀察＝還壓在近 60 個日曆天收盤低。介紹圖紀律／月K還在往上只講現在怎樣，不是買訊。",
    ),
    (
        "lists",
        "如何賣、海選、三種清單",
        "如何賣：最高價＝20日高，對最高溫。只協助出場，不是買訊，不自動賣，不改海選。高點跟熱度都沒了 → 這波先當結束。\n"
        "\n"
        ...
        "\n"
        "三種清單不要搞混\n"
        "觀察＝自選還沒買。持股＝手記真實買入。AI倉＝假錢對照組，平常最多用 1 份，第 3 份留現金。打「持倉」開持股；「持倉報告／模擬持倉」才是 AI倉。",
    ),
    (
        "more",
        "其他按鈕、原因、一天什麼時候動",
        "大盤頁：第二排最左。加權、漲跌家數、法人、台指期、美股上一收盤。美股當天沒開會寫日期與原因。台股國定假或北市全日／上午停班同樣寫原因。\n"
        "當沖：平日 09:00–13:30 才有。隔日沖：尾盤佈局明早。連買區：先選外資／投信／外資+投信，再點天數，上市櫃一起列。\n"
        "AI倉：第二排右二。AI操盤立刻跑一輪不推播。進化只調倍數，不改黃金買點。\n"
        "\n"
        "用平常話問原因：輸入列左邊三條槓有「原因」。為什麼跌／怎麼賣／外資／產業／大盤／海選會對到官方資料。也可傳語音。\n"
        "一天什麼時候動（台灣）：06:30 早報、12:45 尾盤、16:30 官方收盤寫庫、20:00 AI倉模擬買賣不推播。台股休市當日不寄 06:30 與 12:45。",
    ),
    (
        "oops",
        "按錯了怎麼辦",
        "亂按沒關係。下面幾條最常見。\n"
        "\n"
        "1  一打開先按了「刷新上一檔」：還沒查過就直接打代號。打「決策卡」也是同一顆\n"
        "2  「當沖」沒名單：週末／收盤後本來就空。平日 09:00–13:30 才有\n"
        "3  「海選」等很久：掃全市場；不要連按。觀察＝還沒買；持股＝按過記買入才會在\n"
        "4  找不到產業：在圖下面那一排。想問為什麼跌：左邊三條槓點「原因」\n"
        "5  「回報」按下去又反悔：改按其他按鈕即可，不會送出。主選單不見：點四格鍵盤圖示，或打 /menu。畫面怪按第一排最右「回報」。不用給密鑰或密碼",
    ),
)
...
PAGE_PILLS: Dict[str, Tuple[str, ...]] = {
    ...
    "more": ("大盤", "當沖", "隔日沖", "AI倉", "連買區", "原因"),
    "oops": ("刷新上一檔", "當沖沒名單", "海選不要連按", "/menu"),
}
...
def render_picture_guide(out_dir: str | None = None, *, force: bool = False) -> List[str]:
    """產出 6 張長圖；已有快取就沿用。測試／本機對圖用；話筒走 ensure_page。"""
    dest = out_dir or picture_guide_dir()
    os.makedirs(dest, exist_ok=True)
