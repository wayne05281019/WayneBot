...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "第一排（左到右）\n"
        "海選　持股　觀察　刷新　回報　飆大　大盤\n"
        "\n"
        "第二排（左到右）\n"
        "資金　當沖　隔日沖　AI倉　連買區　剛脫離零　洞燭先機\n"
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。大盤＝指數數字頁。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁五角星＝值不值得買（滿五星＝按表該買）。洞燭先機＝還沒點名的次族群；資金看細項佔法人買超％升降，再跟黃金買點對過；沒買點不准發明切入。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
    ...
    (
        "more",
        "其餘按鈕、一天什麼時候動",
        "大盤頁：第一排最右。加權、漲跌家數、法人、台指期、美股上一收盤。美股當天沒開、台股國定假或北市全日／上午停班會寫原因。\n"
    ...
    (
        ...
        "5  「回報」按下去又反悔：改按其他按鈕即可，不會送出。連買選到一半按錯：改按別顆就取消。主選單不見：點四格鍵盤圖示，或打 /menu。畫面怪按「回報」。不用給密鑰或密碼",
    ),
)
