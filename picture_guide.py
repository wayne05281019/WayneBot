...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "說明＝這本使用說明。海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝圈住的獨立觀點（也叫飆客），跟海選無關。裡面三顆：怎麼觀察／去年年底／問一檔。第二排最右一格空白還沒接。\n"
        "打「精簡選單」或「精簡鍵盤」只留說明／海選／持股、觀察／刷新／回報（都是兩個字）；「完整選單」或「完整鍵盤」恢復兩排（含圈住的飆大）。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
        "打 2330、0050、00631L、00981A。不要先按「刷新」。國字打不準就點左邊確認。ETF 可含 L／R／A。"
        "一次兩張：介紹圖（下半高低導航箭頭）／決策卡。圖下可再要完整導航。圖下產業（一張圖卡，細項小框沒抓到不畫）。興櫃四顆一排。K線可滑動對價，可改15分／五日／月線。零股「200股」。"
    ...
    (
        ...
        "黃金買點才是進場表（獲利剛離零且趨勢向上）。海選各桶須趨勢向上，不收空頭。重點觀察、周帶量、止跌同樣擋下坡。20 日高標「少追」。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。優先看、半年高、站上季線。",
    ),
    (
        "sell",
        "如何賣",
