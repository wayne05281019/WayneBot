...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "黃金買點才是進場表：買點＝剛離零且趨勢向上；還在零＝以前的重點觀察，不是買。股名旁五角星＝值不值得買（滿五星＝按表該買）。海選各桶須趨勢向上，不收空頭。優先看、周帶量、半年高、站上季線、止跌不是買訊。大盤狀況末段寫台股產業鏈資金輪動：先機＝佔比升還沒當第一；追當天第一名容易人去樓空。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。",
    ),
    (
        "sell",
        "如何賣",
