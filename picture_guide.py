...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "第二排（左到右）\n"
        "大盤　資金　當沖　隔日沖　AI倉　連買區　剛離零\n"
        "\n"
        "說明＝這本使用說明。海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。剛離零＝收盤前看此刻獲利剛離開 0；股名旁星星＝建議優先看的五檔。\n"
    ...
    (
        ...
        "資金：盤後產業法人買超／賣超，只當佈局對照。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。剛離零：收盤前看此刻獲利剛離零，現價複核、不寫未收盤；星星＝建議五檔。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
