...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "說明＝這本使用說明。海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁星星＝建議優先看的五檔。\n"
    ...
    (
        ...
        "黃金買點才是進場表（獲利剛離零且趨勢向上）。海選各桶須趨勢向上，不收空頭。重點觀察、周帶量、止跌同樣擋下坡。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。優先看、半年高、站上季線。",
    ),
    (
        "sell",
        "如何賣",
    ...
    (
        ...
        "資金：盤後產業法人買超／賣超，只當佈局對照。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。剛脫離零：平日 09:00–13:30 現價複核；非盤中不提供，請按海選看黃金買點。星星＝建議五檔。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
