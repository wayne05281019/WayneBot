...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "資金：盤後產業法人買超／賣超，只當佈局對照。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。剛脫離零：按下去先出獲利為零；下面一排剛離1／2／3。盤中現價複核，收盤後用官方收。獲利為零＝高低卡獲利還是 0，含興櫃，不限海選桶，觀察不是買。剛離1／2／3現在趨勢已向上排前面，還沒向上仍列出不滿五星。五角星＝值不值得買。洞燭先機：第二排最右，還沒點名的次族群看產業鏈佔比升降，推出這型最落後次級兩到三檔；剛好剛離零才標黃金買點。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
