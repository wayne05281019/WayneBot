...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。大盤＝指數數字頁。資金＝盤後產業法人。剛脫離零＝先選剛離／脫離1／2／3／剛為零。剛離＝收盤前看此刻獲利剛離開 0；脫離1＝上一交易日黃金買點、現價還在 5% 內；剛為零＝獲利還是 0，觀察不是買。股名旁五角星＝值不值得買（滿五星＝按表該買，剛為零不算）。洞燭先機＝按下去推出這型最落後次級兩到三檔給你選（不是單檔保證）；剛好剛離零才標黃金買點；打股名沒打準會列出相近的請你點；用完按同一顆「離開洞燭先機」回主選單。資金看主產業／次產業／產業鏈佔比升降，不是整層電子；追當天第一名容易人去樓空。下單進場仍認黃金買點。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
    ...
    (
        ...
        "資金：盤後產業法人買超／賣超，只當佈局對照。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。剛脫離零：按下去先選剛離／脫離1／2／3／剛為零。盤中現價複核，收盤後用官方收。剛為零＝獲利還是 0，觀察不是買。五角星＝值不值得買。洞燭先機：第二排最右，還沒點名的次族群看產業鏈佔比升降，推出這型最落後次級兩到三檔；剛好剛離零才標黃金買點。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
