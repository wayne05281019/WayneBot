...
PAGES: Sequence[Tuple[str, str, str]] = (
    (
        ...
        "3  三張圖出來後（同一則縮圖，一張圖卡），圖下按鈕：產業／觀察／記買入。上市櫃股票另有籌碼、營收、K線、導航圖；ETF 沒有營收鈕\n"
        "\n"
    ...
    (
        ...
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。大盤＝指數數字頁。資金＝盤後產業法人。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁五角星＝值不值得買（滿五星＝按表該買）。洞燭先機＝按下去推出勝率最高這型的黃金買點給你選；打股名沒打準會列出相近的請你點；用完按同一顆「離開洞燭先機」回主選單。資金看主產業／次產業／產業鏈佔比升降，不是整層電子；追當天第一名容易人去樓空。沒黃金買點不准發明切入。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次三張圖",
        "打 2330、0050、00631L、00981A。國字打不準就點左邊確認。ETF 可含 L／R／A。"
        "一次三張放同一則縮圖（一張圖卡）：介紹圖（下半高低導航箭頭）／決策卡／產業圖（產業鏈小框）。點開是 Telegram 允許的最高像素。圖下可再要完整導航。圖下 K線開奇摩股市同一檔日K。零股「200股」。"
    ...
    (
        ...
        "資金：盤後產業法人買超／賣超，只當佈局對照。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。剛脫離零：平日 09:00–13:30 現價複核；非盤中不提供，請按海選看黃金買點。五角星＝值不值得買。洞燭先機：第二排最右，還沒點名的次族群看產業鏈佔比升降再對黃金買點。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
