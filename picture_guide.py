...
PAGES: Sequence[Tuple[str, str, str]] = (
    (
        ...
        "3  兩張圖出來後，圖下按鈕：產業／觀察／記買入／說明。上市櫃股票另有籌碼、營收、K線、導航圖；ETF 沒有營收鈕\n"
        "\n"
    ...
    (
        ...
        "\n"
        "第一排（左到右）\n"
        "說明　海選　持股　觀察　刷新　回報　飆大\n"
        "\n"
        "第二排（左到右）\n"
        "大盤　資金　當沖　隔日沖　AI倉　連買區　剛脫離零\n"
        "\n"
        "說明＝這本使用說明。海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁五角星＝值不值得買（滿五星＝按表該買）。\n"
        "打「精簡選單」或「精簡鍵盤」只留說明／海選／持股、觀察／刷新／回報（都是兩個字）；「完整選單」或「完整鍵盤」恢復兩排（含飆大）。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
