...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "黃金買點＝獲利剛離 0。重點觀察＝近 60 日。低點訊號不是買。月K還在往上不是買訊。興櫃用日均價。",
    ),
    (
        "lists",
        "三種清單不要搞混",
    ...
    (
        ...
        "黃金買點才是進場表（獲利剛離零且趨勢向上）。股名旁五角星＝值不值得買（滿五星＝按表該買）。海選各桶須趨勢向上，不收空頭。重點觀察、周帶量、止跌同樣擋下坡。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。優先看、半年高、站上季線。",
    ),
    (
        "sell",
        "如何賣",
