...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。大盤＝指數數字頁。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁五角星＝值不值得買（滿五星＝按表該買）。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
