...
PAGES: Sequence[Tuple[str, str, str]] = (
    (
        "cover",
        "第一次用，先做這三步",
        "這本說明給手機看。一次只看一張。現在共 9 張。按「第 2 張」看下一張。\n"
        "\n"
        "1  點四格鍵盤圖示叫出兩排。不見就打 /menu\n"
        "2  直接打代號看圖，例如 2330、0050、0052、00631L、00981A\n"
        "3  兩張圖出來後（同一則縮圖，一張圖卡），圖下按鈕：產業／導航圖／觀察／記買入。上市櫃股票另有籌碼、營收、K線；ETF 沒有營收鈕\n"
        "\n"
    ...
    (
        ...
        "\n"
        "第一排（左到右）\n"
        "海選　持股　觀察　飆大　台股大盤　資金輪動\n"
        "\n"
        "第二排（左到右）\n"
        "當沖　隔日沖　AI倉　連買區　獲利為零　洞燭先機\n"
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。台股大盤＝指數數字頁。資金輪動＝盤後產業法人。獲利為零＝按下去直接出今天高低卡獲利還是 0 的檔（含興櫃，不限海選桶，觀察不是買，只列前8檔）。進去後下方鍵盤左到右脫離1／脫離2／脫離3＝1／2／3 個交易日前高低卡第一天離零（實綠：昨獲利貼零、今離開 0），現在獲利不是 0 就列出，不卡 5%、含興櫃、只列前8檔；現在趨勢已向上排前面，還沒向上最高兩星；靠近20高標少追。滿五星＝按表該買；四星＝趨勢已向上、獲利還在帶、沒少追。今天黃金買點仍在海選。股名旁五角星＝值不值得買（獲利為零不算）。洞燭先機＝按下去推出這型最落後次級兩到三檔給你選（不是單檔保證）；剛好剛離零才標黃金買點；打股名沒打準會列出相近的請你點；用完按同一顆「離開洞燭先機」回主選單。資金輪動看主產業／次產業／產業鏈佔比升降，不是整層電子；追當天第一名容易人去樓空。下單進場仍認黃金買點。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
        "打 2330、0050、00631L、00981A。國字打不準就點左邊確認。ETF 可含 L／R／A。"
        "一次兩張放同一則縮圖（一張圖卡）：介紹圖／高低溫度卡。點開高畫質。產業鈕出產業圖（產業鏈小框）；導航圖鈕出 180 日高低。K線開奇摩股市同一檔日K。零股「200股」。"
        "黃金買點＝剛離零才進場；還在零＝觀察不是買。低點訊號不是買。月K還在往上不是買訊。興櫃用日均價。",
    ),
    (
        "lists",
        "三種清單不要搞混",
    ...
    (
        ...
        "1  左鍵＝這檔完整圖；右 + ＝觀察\n"
        "2  靠近 20 日收盤高會標「少追」，不是叫立刻買\n"
        "黃金買點才是進場表：買點＝剛離零且趨勢向上；還在零＝以前的重點觀察（60低超跌），不是買。股名旁五角星＝值不值得買（滿五星＝按表該買）。海選畫面只出這一欄。周帶量、半年高、站上季線、止跌、優先看不是買訊、不進海選畫面。黃金買點仍不收空頭。大盤狀況寫加權、美股，以及布蘭特原油（美元/桶）、美元指數、美元兌台幣；沒真數不上。末段寫台股產業鏈資金輪動：先機＝佔比升還沒當第一；追當天第一名容易人去樓空。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。",
    ),
    (
        "sell",
        "如何賣",
        ...
        "\n"
        "紀律句對當日格子。高點跟熱度都沒了 → 這波先當結束。升溫靠近20日高不會寫成熱度退了。\n"
    ...
    (
        ...
        "\n"
        "不要跌破前波低，也不要跌破昨天低，否則底底低。獲利還沒離開 0 先不要動作。\n"
        "已經買、跌破該低 → 停損觀望。紅箭頭不是買訊。進場仍看高低卡黃金買點。",
    ),
    (
        "more",
        "其餘按鈕、一天什麼時候動",
        "台股大盤頁：第一排「台股大盤」。加權、漲跌家數、法人、台指期、美股上一收盤、布蘭特／美元指數／美元兌台幣。美股當天沒開、台股國定假或北市全日／上午停班會寫原因。\n"
        "資金輪動：盤後產業法人買超／賣超，只當佈局對照。盤中另列成交佔比升／降（不是法人）。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。獲利為零：按下去直接出獲利還是 0 的前8檔；下方鍵盤左到右脫離1／2／3（實綠離零、前8檔）。盤中現價複核，收盤後用官方收。獲利為零＝高低卡獲利還是 0，含興櫃，不限海選桶，觀察不是買。脫離1／2／3現在趨勢已向上排前面，還沒向上最高兩星；靠近20高標少追。滿五星＝按表該買。洞燭先機：第二排最右，還沒點名的次族群看產業鏈佔比升降，推出這型最落後次級兩到三檔；剛好剛離零才標黃金買點。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
        "也可以傳語音，聽成文字後跟打字同一條。按了飆大之後打字或按麥克風講都會回。雲端要有聽寫金鑰才聽得懂；沒金鑰請改打字。\n"
        "一天什麼時候動（台灣）：06:30 早報（早上海選）、12:45 尾盤（對今早名單報現價，先講現在要做什麼）、16:30 官方收盤寫庫（興櫃日均價也在這時寫獨立表）、20:00 AI倉模擬買賣不推播。台股休市當日不寄 06:30 與 12:45。",
    ),
    (
        "oops",
        "按錯了怎麼辦",
        "亂按沒關係。下面幾條最常見。\n"
        "\n"
        "1  還沒查過就直接打代號，例如 2330\n"
        "2  「當沖」沒名單：週末／收盤後本來就空。平日 09:00–13:30 才有\n"
        "3  「海選」等很久：掃全市場；不要連按。觀察＝還沒買；持股＝按過記買入才會在\n"
        "4  找不到產業或報導或K線：在圖下面最上那一排。K線開奇摩股市同一檔日K。想看怎麼賣：打代號，圖底下會寫\n"
        "5  連買選到一半按錯：改按別顆就取消。主選單不見：點四格鍵盤圖示，或打 /menu。不用給密鑰或密碼",
    ),
)
...
PAGE_PILLS: Dict[str, Tuple[str, ...]] = {
    "menu": ("海選", "持股", "觀察", "飆大", "台股大盤", "資金輪動"),
    "lists": ("觀察", "持股", "AI倉", "成交", "復盤", "賣出"),
    "sell": ("如何賣", "20日高", "最高溫", "不是買訊"),
    "lowbuy": ("低點≠買", "獲利貼0", "勿破前低", "不是買訊"),
    "more": ("台股大盤", "資金輪動", "當沖", "隔日沖", "連買區", "獲利為零", "洞燭先機"),
    "oops": ("打代號", "當沖沒名單", "海選不要連按", "連買", "/menu"),
}
