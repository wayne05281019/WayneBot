...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "1  輸入列打代號或股名，送出就出圖\n"
        "2  一次出三張，點縮圖可放大\n"
        "3  撞名或國字打不準會列出相近的，點左邊確認才出圖\n"
        "\n"
        "KY 可只打前面。找不到再打代號。ETF 可含 L／R／A；海選名單仍只有股票／KY。",
    ),
    (
        "charts",
        "查一檔：一次三張圖",
    ...
    (
        "market",
        "大盤頁",
        "第二排、隔日沖右邊。加權、漲跌家數、法人、台指期、美股上一收盤，下面是橫式日K。\n"
        "\n"
        "1  加權用證交所即時或庫內官方收盤；沒接到的市場不寫\n"
        "2  美股塊寫上一收盤日狀況，不是假裝還在開盤\n"
        "3  美股當天沒開（例如勞動節）會寫日期與原因，並附上一收盤數字\n"
        "4  台股國定假或北市全日／上午停班，同樣寫日期與原因，並附上一收盤\n"
        "5  只讀、不寫進資料庫。不影響 16:30 官方收盤寫庫\n"
        "\n"
        "這頁沒有循環短圖。按大盤看數字與日K即可。",
    ),
    (
        "streak",
        "連買區",
    ...
    (
        ...
        "\n"
        "台股休市當日不寄 06:30 海選與 12:45 尾盤。盤中查股用證交所即時價（不寫庫）。16:30 後以庫內官方收盤為準。",
    ),
    (
        "oops",
        "按錯了怎麼辦",
...
PAGE_PILLS: Dict[str, Tuple[str, ...]] = {
    ...
    "market": ("加權", "櫃買", "台指期", "美股上一收盤"),
    "daytrade": ("當沖 09:00–13:30", "隔日沖看尾盤", "週末休市改看隔日沖"),
