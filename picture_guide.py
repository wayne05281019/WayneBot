...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。大盤＝指數數字頁。資金＝盤後產業法人。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁五角星＝值不值得買（滿五星＝按表該買）。洞燭先機＝還沒點名的次族群；資金看這檔自己的主產業／次產業／細項佔法人買超％升降，不是整層電子；打代號看能不能留；沒黃金買點不准發明切入。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
    ...
    (
        ...
        "黃金買點才是進場表：買點＝剛離零且趨勢向上；還在零＝以前的重點觀察，不是買。股名旁五角星＝值不值得買（滿五星＝按表該買）。海選各桶須趨勢向上，不收空頭。優先看、周帶量、半年高、站上季線、止跌不是買訊。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。",
    ),
    (
        "sell",
        "如何賣",
