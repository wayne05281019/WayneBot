...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "\n"
        "第二排（左到右）\n"
        "資金　當沖　隔日沖　AI倉　連買區　剛脫離零\n"
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。刷新＝只重畫上一檔，不是海選。打「決策卡」或「刷新上一檔」也行。回報＝畫面怪告訴偉權。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。大盤＝指數數字頁。剛脫離零＝收盤前看此刻獲利剛離開 0；股名旁五角星＝值不值得買（滿五星＝按表該買）。\n"
        "打「精簡選單」或「精簡鍵盤」只留海選／持股／觀察、刷新／回報（都是兩個字）；「完整選單」或「完整鍵盤」恢復兩排（含飆大）。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：一次兩張圖",
        "打 2330、0050、00631L、00981A。不要先按「刷新」。國字打不準就點左邊確認。ETF 可含 L／R／A。"
        "一次兩張：介紹圖（下半高低導航箭頭）／決策卡。圖下可再要完整導航。圖下產業（一張圖卡，細項小框沒抓到不畫）。興櫃圖下可滑 K線。K線可滑動對價，可改15分／五日／月線。零股「200股」。"
