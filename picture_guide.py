...
PAGES: Sequence[Tuple[str, str, str]] = (
    (
        ...
        "3  三張圖出來後（介紹／高低卡同一則，再送大量區），圖下按鈕：產業／導航圖／觀察／記買入。上市櫃股票另有籌碼、營收、K線；ETF 沒有營收鈕\n"
        "\n"
    ...
    (
        ...
        "\n"
        "第二排（左到右）\n"
        "當沖　隔日沖　AI倉　連買區　剛脫離零　洞燭先機\n"
        "\n"
        "海選＝昨天收盤掃出的名單。持股＝你手記的真實買入。觀察＝還沒買的自選。飆大＝即時對話窗口（也叫飆客）；打字或按麥克風講都會自動回，用完按同一顆「離開飆大」，跟海選無關。台股大盤＝指數數字頁。資金輪動＝盤後產業法人。剛脫離零＝按下去直接出昨獲利貼零、今天離開 0 的檔（實綠第一天；含興櫃，不限海選桶，現在獲利不是 0 就列出，不卡 5%，只列前8檔）。沒有子鍵。現在趨勢已向上排前面，還沒向上最高兩星；靠近20高標少追。滿五星＝按表該買；四星＝趨勢已向上、獲利還在帶、沒少追。今天黃金買點仍在海選。股名旁五角星＝值不值得買。洞燭先機＝按下去推出這型最落後次級兩到三檔給你選（不是單檔保證）；剛好剛離零才標黃金買點；打股名沒打準會列出相近的請你點；用完按同一顆「離開洞燭先機」回主選單。資金輪動看主產業／次產業／產業鏈佔比升降，不是整層電子；追當天第一名容易人去樓空。下單進場仍認黃金買點。兩人同一套完整兩排，沒有精簡鍵盤。還沒查過請直接打代號（股票或 ETF）。",
    ),
    (
        "lookup",
        "查一檔：介紹／高低卡／大量區",
        "打 2330、0050、00631L、00981A。國字打不準就點左邊確認。ETF 可含 L／R／A。"
        "上市／上櫃／興櫃一律三張：一次兩張放同一則縮圖（介紹圖／高低溫度卡），接著再送大量區專圖（近窗爆大量日高低，非買訊）。點開高畫質。產業鈕出產業圖（產業鏈小框）；導航圖鈕出 180 日高低。K線開奇摩股市同一檔日K。零股「200股」。"
    ...
    (
        ...
        "資金輪動：盤後產業法人買超／賣超，只當佈局對照。盤中另列成交佔比升／降（不是法人）。當沖平日 09:00–13:30 才有。隔日沖尾盤佈局明早。剛脫離零：按下去直接出昨獲利貼零、今天離開 0 的前8檔（實綠第一天，含興櫃，不限海選桶）。沒有子鍵。盤中現價複核，收盤後用官方收。現在趨勢已向上排前面，還沒向上最高兩星；靠近20高標少追。滿五星＝按表該買。洞燭先機：第二排最右，還沒點名的次族群看產業鏈佔比升降，推出這型最落後次級兩到三檔；剛好剛離零才標黃金買點。連買區直接選外資／投信／外資+投信。興櫃沒法人表、不在連買區；興櫃請按海選再選。按鈕只在訊息下。\n"
...
PAGE_PILLS: Dict[str, Tuple[str, ...]] = {
    ...
    "more": ("台股大盤", "資金輪動", "當沖", "隔日沖", "連買區", "剛脫離零", "洞燭先機"),
