...
PAGES: Sequence[Tuple[str, str, str]] = (
    ...
    (
        ...
        "黃金買點才是進場表：買點＝剛離零且趨勢向上；還在零＝以前的重點觀察（60低超跌），不是買。股名旁五角星＝值不值得買（滿五星＝按表該買）。海選畫面只出這一欄。周帶量、半年高、站上季線、止跌、優先看不是買訊、不進海選畫面。黃金買點仍不收空頭。大盤狀況末段寫台股產業鏈資金輪動：先機＝佔比升還沒當第一；追當天第一名容易人去樓空。興櫃獨立名單：按海選後選興櫃，也可打興櫃海選／興櫃名單，不混進上市櫃。",
    ),
    (
        "sell",
        "如何賣",
    ...
    (
        "more",
        "其餘按鈕、一天什麼時候動",
        "大盤頁：第一排「大盤」。加權、漲跌家數、法人、台指期、美股上一收盤。美股當天沒開、台股國定假或北市全日／上午停班會寫原因。\n"
...
PAGE_PILLS: Dict[str, Tuple[str, ...]] = {
    ...
    "more": ("大盤", "資金輪動", "當沖", "隔日沖", "連買區", "獲利為零", "洞燭先機"),
