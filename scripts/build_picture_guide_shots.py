...
def _ring(
    ...
    # 白邊再套紅圈，綠底上也看得清
    draw.ellipse((cx - rx - 2, cy - ry - 2, cx + rx + 2, cy + ry + 2), outline=WHITE, width=width + 3)
...
def _caption_bar(im: Image.Image, text: str) -> Image.Image:
    ...
    d.text((pad, im.height + 12), text, font=font, fill=ACCENT)
    return out
...
def build(src: str = SRC) -> list[str]:
    ...
    if os.path.isfile(kb_src):
        ...
        # 量過的座標（聊天區 901×715、從 y=480 切）：
        # 兩排鈕約 x=340–660、y=75–155；四格圖示約 (645, 172)
        sl = _kb_slice(chat)
        d = ImageDraw.Draw(sl)
    ...
    if os.path.isfile(help_src):
        ...
        sl = chat.crop((0, 0, chat.width, 340))
        d = ImageDraw.Draw(sl)
    ...
    if os.path.isfile(alb_src):
        ...
        sl = chat.crop((0, 0, chat.width, 500))
        d = ImageDraw.Draw(sl)
        ...
        bot = chat.crop((0, 400, chat.width, chat.height))
