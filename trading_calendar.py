...
def leave_zero_closed_message(now=None) -> str:
    """非盤中按獲利為零：不抓現價，改用官方收；可選脫離1–3。"""
    when = next_tw_session_when(now)
    return (
        "目前非盤中，不抓現價，名單用官方收。\n"
        "可選脫離1／2／3。獲利為零＝獲利還是 0，觀察不是買。含興櫃，不限海選桶。\n"
        f"{when} 開盤後再按，才用現價複核。"
    )
...
def overnight_list_heading(phase: str) -> tuple[str, str]:
    ...
    if phase == "pre":
        return (
            "⚡ 隔日沖候選（開盤前預覽）",
            "開盤前預覽：昨收強勢節奏名單，不是進場表。"
            "09:00 後再依盤中價複核。未持倉僅供觀察。",
        )
    if phase == "weekend":
        return (
            "⚡ 隔日沖候選（休市參考）",
            "休市參考：上個交易日強勢收盤節奏名單，不是叫你現在買。",
        )
    return (
        "⚡ 隔日沖候選（收盤後參考）",
        "收盤後參考：今日強勢收盤節奏名單，不是進場表。"
        "尾盤買進時段已過；若未持倉僅供觀察。",
    )
