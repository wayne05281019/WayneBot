...
def _badge_style(text: str):
    ...
    if any(k in t for k in ("均量", "過小", "整理", "格局")):
        return C["navy"], C["white"]
    return C["white"], C["neutral_fg"]


def _status_badge_colors(bg, fg):
    """表格狀態格改實心白字，不要整格淡底。"""
    C = _CARD
    if fg == C["white"] and bg not in (C["white"], C["panel"], C["neutral_bg"], None, ""):
        return bg, fg
    if bg in (C["hi_fill"], C["temp_hot_bg"], C["temp_warm_bg"], C["vol_hi_bg"]):
        return C["pill_hi"], C["white"]
    if bg in (C["lo_fill"], C["lo_hit_fill"]):
        return C["pill_lo"], C["white"]
    if bg in (C["temp_compress_bg"], C["price_not_low_bg"], C["navy"], C["pill_hi"], C["pill_lo"]):
        return bg, C["white"]
    return bg, fg
...
def fit_rows(rows, row_w, fig_w, *, fa=12.0, fb=15.0, gap=5.5, weight=800, floor=9.5, scale=1.0):
    """同一區塊各列共用字級：取各列需求裡最小的那組，字高一致、右對齊的數字才會對齊。

    rows 是 (標題或標題備選, 數值) 的序列。回傳 (每列採用的標題, 標題字級, 數值字級)。
    scale>1 時用放大後的字去量寬，回傳仍是設計字級（繪製端再乘 scale）。
    """
    s = float(scale) if scale else 1.0
    fa, fb, floor = fa * s, fb * s, floor * s
    rows = list(rows)
    if not rows:
        return [], fa / s, fb / s
    picked = [
...
def render_decision_card_png(card: dict, save_path: str) -> str:
    ...
    weights = [11.6, 11.8, 8.6, 10.2, 15.4, 12.2, 9.2, 12.0]
    pill_cols = {3, 4}
    span = 100 - 2 * pad_x
    xs = [pad_x]
    for wgt in weights:
    ...
    for i, h in enumerate(headers):
        ...
        ax.text((xs[i] + xs[i + 1]) / 2, tbl_top - hdr_h / 2, h, fontproperties=_fp(12.4, "bold"),
    ...
    for row_i, (_, r) in enumerate(table.iterrows()):
        ...
        for i, val in enumerate(vals):
            ...
            if i == 2 and fills[i] in (C["lo_fill"], C["pill_lo"]):
                ...
                _pill(ax, cx, cy, val, C["pill_lo"], C["white"], w=pill_w, h=body_h * 0.70, fs=12.5)
            elif i == 2 and fills[i] == C["lo_hit_fill"]:
                pill_w = min(tw(val, 12) + 3.2, col_w * 0.92)
                _pill(ax, cx, cy, val, fills[i], fgs[i], w=pill_w, h=body_h * 0.70, fs=12.5)
            elif i in pill_cols:
                if is_blank_card_signal(val):
                    ax.text(cx, cy, "No",
                            fontproperties=_fp(12), color=C["ink_mute"], ha="center", va="center", zorder=3)
                    ...
                    _status_pill(cx, cy, val, fills[i], fgs[i], w=pill_w,
                                 h=body_h * 0.74, fs=12.5)
            else:
                if i == 5:
                    px_fs = 13.5
                elif i == 1 and len(str(val)) >= 7:
                    px_fs = 11.5
                else:
                    px_fs = 12.5
...
def render_first_glance_png(stock_id: str, card: dict, tape: dict, save_path: str, db_path: str = None) -> str:
    ...
    ohlc_items = [
        ...
        ("高", _fmt_price(ohlc_src.get("high") or card.get("high"))),
        ("低", _fmt_price(ohlc_src.get("low") or card.get("low"))),
    ]
...
def _lerp_hex(a, b, t: float):
    ar, ag, ab = mcolors.to_rgb(a)
    br, bg, bb = mcolors.to_rgb(b)
...
def draw_from_ohlc(
    ...
    *,
    ...
    for i in range(n):
        ...
        # 每根 K 每側只留一支：高側 h20 > 脫離 > 接近；低側 60低 > 20低 > 脫離 > 接近。
        # 相鄰兩日弱訊號再擋一次，寬頭才不會跟隔壁糊成一坨。
        dn_pick = None
