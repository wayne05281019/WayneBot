...
_CASES: List[Tuple[re.Pattern[str], str]] = [
    ...
    (
        ...
        "2026-04-22 公開 IC 設計主線點創意／力旺／世芯，當天沒點發哥；官方聯發科當日收 2295 接近漲停。"
        "5 月起才把發哥當平台龍頭。聯發科不在 4/16 那份可抱到明年名單（那份是台積電、台達電、台光電、旺矽、穎崴、奇鋐）。"
    ...
    (
        re.compile(r"(南亞)(?!科)|(?<!\d)1303(?!\d)"),
        "南亞 1303 不是南亞科 2408，也不是 4/16 可抱到明年名單。"
        "公開 1709 對不到「適合上班族長期抱」這六字。"
        "2025-11-18：集團作帳、南亞科母公司可介入。"
        "2026-06-22：大型權值短波段／隔日沖，位階當時最低之一。"
        "2026-06-30：B 波可操作有南亞；F10 長線要反市場能抱得住，不是把南亞寫進 4/16。"
        "2026-07-15 樓下：第一批上車約 40%。"
        "2026-07-22 樓下：和 AI 討論確認已轉型成 PCB 或 AI 材料。"
        ...
        "後來再過 217 是另一段，不准寫成當時不該跑。長抱不是死抱。"
        "記憶體主線講的是南亞科 2408。不是買訊。",
    ),
]
