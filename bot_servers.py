...
HELP_TOPICS = {
    "guide": (
        ...
        "\n"
        "<b>五、AI倉是對照組（不是真持股）</b>\n"
        "主選單第一排最右 <b>AI倉</b>（也可打 AI模擬倉）：用假錢、照紀律長期買，讓你對照「若只按表走會怎樣」。重點是對照組，不是幫你下單、也不會改程式。\n"
        "它優先買黃金買點，槽還空才看重點觀察等；當沖不隔夜；少追／美股逆風不買。停損約 −7%、停利約 ＋8%。\n"
        "每晚 20:00 自動模擬（不推播）；要看成績請自己按 AI倉。訊息下方 <b>AI操盤</b> 可立刻跑一輪。點下方 <b>AI</b> 看完整說明。\n"
        "\n"
        "<b>六、打股名或代號（例：南亞、2324）</b>\n"
        "會一次出三張圖（相簿）：<b>介紹圖</b> → <b>決策卡</b> → <b>導航圖</b>。點縮圖可放大。\n"
        "圖下方按鈕（查完才出現，不是主選單那兩排）：\n"
        "• <b>籌碼</b>：三大法人買賣超圖\n"
        "• <b>營收</b>：月營收、季報毛利\n"
        ...
        "• <b>➕</b>＝加入觀察\n"
        "• 區底 <b>一鍵傳 LINE</b>：生成介紹圖／決策卡。中轉頁可<b>勾選要傳哪幾檔</b>（介紹圖＋決策卡一組），再複製名單或傳勾選的圖到 LINE\n"
        ...
        "盤中查股用證交所即時價（不寫庫）；13:30～16:30 融合前若即時價空白，查股會用奇摩參考價，16:30 後以庫內官方收盤為準。\n"
        "\n"
        "<b>十一、資料正確性（最重要）</b>\n"
        "庫內日 K 只寫官方融合後的收盤；盤中即時價與奇摩僅供查股顯示，不寫進資料庫。\n"
        "假 K、週末殘列、漲跌幅與前日收盤不符，啟動與 16:30 融合後會自動清掉或重算。\n"
        "你挑股認黃金買點欄：若名單上的檔，打開決策卡獲利格不是剛離零，請按 <b>回報</b> 或貼給偉權。\n"
        "\n"
        "<b>十二、多人使用（家人各用各的）</b>\n"
        "每人用自己的話筒帳號跟機器人<b>私聊</b>：持股、觀察、成交、連買、查股出圖都<b>各看各的</b>，不會刪到對方訊息。\n"
    ...
    "row1": (
        ...
        "• <b>怎麼用</b>：先打一次股名或代號看圖，之後盤中常按這顆刷新即時現價、量排名。\n"
        "• <b>沒反應</b>：若還沒查過任何股，會請你先打代號或從觀察清單點一檔。\n"
        "• <b>注意</b>：這是單檔快捷鍵，不是海選黃金買點名單。\n"
        "\n"
        "<b>② 當沖</b>\n"
        "• <b>是什麼</b>：盤中即時複核的當沖候選（漲幅約 2%～8.5%）。\n"
        "• <b>怎麼用</b>：平日 <b>09:00–13:30</b> 按；列出保險進場、停利、停損參考價。\n"
        "• <b>收盤後</b>：按了不會出名單（沒有盤中即時價）。\n"
        "• <b>會是空的</b>：美股隔夜大跌、恐慌指數高時故意不列，避免硬沖。\n"
        "\n"
        "<b>③ 持股</b>\n"
        ...
        "\n"
        "<b>⑤ 海選</b>\n"
        "• <b>是什麼</b>：依<b>昨收</b>掃全市場的佈局名單（黃金買點、重點觀察、優先看、周帶量等）。\n"
        "• <b>怎麼用</b>：按一次等 2～5 分鐘，完成後分類推送；勿連按以免排隊。\n"
        "• <b>自動版</b>：平日 06:30 只寄黃金買點／重點觀察／優先看／周帶量（有名單才寄）；12:45 有尾盤可切版。\n"
        "• <b>注意</b>：不是盤中即時掃描；當沖／隔日沖要另按第二排按鈕。\n"
        "\n"
        "<b>⑥ AI倉</b>\n"
        "• <b>是什麼</b>：長期照紀律買的<b>對照組</b>（假錢、最多 3 檔），用來對照你手記持股，不是真下單。\n"
        "• <b>怎麼用</b>：按進去看現金／持倉／停損停利；點股名看這檔介紹圖與決策卡。\n"
        "• <b>AI操盤</b>：在 AI倉 頁訊息下方，立刻依海選跑一輪模擬買賣（不推播）。\n"
        "• <b>跟持股</b>：持股＝你手記的真實買入；AI倉＝假錢對照組，不會傳到偉權改碼對話。"
    ),
    "row2": (
        "<b>第二排按鈕（左→右）</b>\n"
        "\n"
        ...
        "\n"
        "<b>② 大盤</b>\n"
        "• <b>是什麼</b>：加權指數、漲跌家數、三大法人、台指期日盤／夜盤、前一晚美股與盤後期貨，並附橫式日K（對齊個股導航圖）。\n"
        "• <b>怎麼用</b>：隨時按；只讀庫內資料，不會觸發匯入或改寫行情。庫沒夜盤時會讀期交所盤後（只顯示）。\n"
        "• <b>跟海選</b>：早報／海選第一則就是白話大盤總覽；這頁給你看數字與圖。\n"
        "\n"
        "<b>③ 資金</b>\n"
        ...
        "• 再點連買天數（有 25 天就會出現 25）。上市櫃一起列，不再分市場。\n"
    ...
    "ai": (
        ...
        "\n"
        "<b>模擬規則（簡要）</b>\n"
        "• 每人本金 50 萬虛擬、最多同時 3 檔，優先從<b>黃金買點</b>補滿，槽還空才看重點觀察／優先看／隔日沖／周帶量\n"
        "• 當沖不隔夜；靠近20日高（少追）、美股電子逆風的不買；停損約 -7%、停利約 +8%\n"
        "• 這是<b>對照組</b>，不會動你的真實持股，也不會真的下單，也不會自動改程式。\n"
        "\n"
        "<b>跟真實持股的差別</b>\n"
        "• <b>持股</b>＝你手動記的買入　• <b>AI模擬倉</b>＝假錢對照組（每人一套，家人也各看各的）。"
    ),
    "decision": (
        "<b>決策卡按鈕</b>\n"
        "主選單第一顆。盤中刷新上一檔的高低決策卡與即時價量。\n"
    ...
    "stock": (
        ...
        "8 產業說明＝官方產業別＋同業月營收／毛利率中位＋這族法人連買／連賣，講人話；不是內幕\n"
    ...
    "streak": (
        "<b>連買區怎麼用</b>\n"
        "主選單第二排「連買區」。\n"
        "先選哪一種：<b>外資</b>、 <b>投信</b>、或<b>外資+投信</b>\n"
        "（外資+投信＝同一天兩家都買超才算一天）。\n"
        "再點連買天數。上市櫃一起列，不用再選市場。\n"
        "天數只列出「剛好有股票」的連買天數（最長會標在訊息裡）。\n"
        "點 6 就只看剛好連買 6 天的股票。\n"
        "每檔顯示代號、股名、N 日連買幾張、佔 N 日總成交％。\n"
        "點股名＝一般查股；按<b>籌碼</b>核對官方法人表。"
    ),
}
