...
HELP_TOPICS = {
    "guide": (
        ...
        "2　<b>直接打四碼</b>看圖，例如 <code>2330</code>（不要先按「決策卡」）\n"
    ...
    "row1": (
        ...
        "\n"
        "<b>⑤ 海選</b>\n"
        "• 是什麼：依昨收掃全市場的佈局名單（黃金買點、重點觀察、優先看、周帶量等）。\n"
        "• 怎麼用：按一次等 2～5 分鐘，完成後分類推送；勿連按以免排隊。\n"
        "• 自動版：平日 06:30 只寄黃金買點／重點觀察／優先看／周帶量（有名單才寄）；12:45 有尾盤可切版。\n"
        "• 注意：不是盤中即時掃描；當沖／隔日沖要另按主選單按鈕。\n"
        "\n"
        "<b>⑥ AI倉</b>\n"
        "• 是什麼：長期照紀律買的對照組（假錢、最多 3 檔），用來對照你手記持股，不是真下單。\n"
        "• 怎麼用：按進去看現金／持倉／停損停利；點股名看這檔介紹圖與決策卡。\n"
        "• AI操盤：在 AI倉 頁訊息下方，立刻依海選跑一輪模擬買賣（不推播）。\n"
        "• 跟持股：持股＝你手記的真實買入；AI倉＝假錢對照組，不會傳到偉權改碼對話。"
    ),
    "row2": (
        "<b>第二排按鈕（左→右）</b>\n"
        "\n"
        ...
        "\n"
        "<b>② 大盤</b>\n"
        "• 是什麼：加權指數、漲跌家數、三大法人、台指期日盤／夜盤、前一晚美股與盤後期貨，並附橫式日K。\n"
    ...
    "ai": (
        ...
        "\n"
        "<b>模擬規則（簡要）</b>\n"
        "• 每人本金 50 萬虛擬、最多同時 3 檔，優先從黃金買點補滿，槽還空才看重點觀察／優先看／隔日沖／周帶量\n"
    ...
    "screen": (
        ...
        "06:30 早報第一則是大盤狀況（美股＋台指期夜盤＋白話連動），接著只寄黃金買點／重點觀察／優先看／周帶量（優先看沒名單就跳過）；半年高／站上季線／止跌請按主選單「海選」看完整。\n"
    ...
    "daytrade": (
        ...
        "只在平日 <b>09:00–13:30 盤中</b> 按才有意義（即時複核漲幅 2%～8.5%）；收盤後、週末按不會出名單。\n"
    ...
    "stock": (
        ...
        "• 月線一句（還在往上／已走空／在整理）＝用每月收盤看現在在哪一段，不是買訊，不改海選\n"
        ...
        "• 產業說明＝官方產業別＋同業月營收／毛利率中位＋這族法人連買／連賣，講人話；不是內幕\n"
    ...
    "buy": (
        ...
        "\n"
        "打完會進「持股」。要取消就改按其他按鈕。\n"
        "一張＝1000 股。觀察頁的「買入」跟這顆一樣。"
    ),
    "pick": (
        "<b>查某一檔</b>\n"
        "直接打股名或四碼，例如 <b>南亞</b>、<b>2324</b>。\n"
        "\n"
        "不要先按「決策卡」——那顆是刷新上一檔。\n"
        "一次出三張圖：介紹圖 → 決策卡 → 導航圖。\n"
        "找不到：再打一次四碼；名稱撞名時點藍字或左邊股名。"
    ),
    "flow": (
        "<b>資金移動怎麼用</b>\n"
    ...
    "oops": (
        ...
        "\n"
        "<b>一打開先按了「決策卡」</b>\n"
        "那顆是刷新上一檔。還沒查過就<b>直接打四碼</b>，例如 <code>2330</code>。\n"
        "\n"
        "<b>「當沖」沒名單</b>\n"
...
class WayneTelegramBot:
    ...
    async def _refresh_reply_menu(self, message, *, uid: str = "", silent: bool = False):
        ...
        text = (
            "兩排已更新：第一排最右 AI倉，第二排最右回報。點輸入列旁邊四格 ⌨️。"
            if silent
            else "主選單已掛上（輸入列旁邊四格鍵盤圖示展開兩排；第一排最右 AI倉，第二排最右回報）。"
        )
        try:
            pin = await message.reply_text(text, reply_markup=self._reply_menu())
            actor = self._actor_key(message, uid=uid)
