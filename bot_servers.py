...
HELP_TOPICS = {
    ...
    "row2": (
        ...
        "\n"
        "<b>⑦ 洞燭先機</b>\n"
        "• 是什麼：飆大教過的找法當參考主因（還沒熱、第一名誰先過前高、從底部找落後），再長期記資金輪動（三大法人張）看有沒有慢慢匯入、準備點火。\n"
        "• 怎麼用：按進去看此刻最像哪一族、為什麼、近幾日法人張。這族再跟全市場黃金買點對過，只推薦有買點的檔。沒有黃金買點不准發明切入。還在零只觀察。盤中未收不當官方收。\n"
    ...
    "dongzhu": (
        ...
        "再長期記資金輪動（日 K 三大法人張，不抓分點）：連日小買超＝慢慢匯入、準備點火。"
        "當下他點名的熱門族不當新族群；修正沉寂一陣子後仍可能再被找到。\n"
        "對官方完整日 K，盤中未收不當官方收。"
        "告訴你此刻最像哪一族、為什麼、近幾日法人張，再把這族跟全市場黃金買點對過。"
