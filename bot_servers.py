...
HELP_TOPICS = {
    ...
    "row2": (
        ...
        "\n"
        "<b>⑦ 洞燭先機</b>\n"
        "• 是什麼：飆大教過的找法當參考主因（還沒熱、第一名誰先過前高、從底部找落後），對五件只落在這族（波浪不數個股）。資金看 CMoney 細項佔當日法人買超％怎麼變：佔比升＝流入、佔比降＝流出，張數會被熱門族蓋過所以不拿來排名。\n"
        "• 怎麼用：按進去看此刻最像哪一族、為什麼、細項佔比路徑、對五件。這族再跟全市場黃金買點對過，只推薦有買點的檔。沒有黃金買點不准發明切入。還在零只觀察。盤中未收不當官方收。\n"
        "• 不是什麼：不是當下熱門族、不是他當下點名、不是買訊、不進海選。紅箭頭不是買訊。打「洞燭」或「先機」也行。"
    ),
    "market": (
        "<b>大盤按鈕</b>\n"
    ...
    "dongzhu": (
        "<b>洞燭先機怎麼用</b>\n"
        "第二排最右。飆大教過的找法當參考主因：次族群還沒熱、第一名誰先過前高、從底部找落後。"
        "對五件只落在這族：底部這層（不數浪）、形態還沒過前高、量價落後檔量起來、關鍵K＝官方收、碎形＝第一名還沒先過。"
        "資金流騙不了人：單位＝CMoney 細項佔當日法人買超％與％變化，流入＝佔比升、流出＝佔比降；不比張數、不抓分點。"
        "當下他點名的熱門族不當新族群；修正沉寂一陣子後仍可能再被找到。\n"
        "對官方完整日 K，盤中未收不當官方收。"
