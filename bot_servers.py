...
HELP_TOPICS = {
    "guide": (
        ...
        "\n"
        "其他海選欄（優先看／周帶量／止跌等）不是買訊。盤中請打開該檔決策卡對獲利格。海選名單是官方收盤掃的。收盤前要看此刻剛脫離零，按第二排最右「剛脫離零」（現價複核，不寫未收盤）。海選各桶都要趨勢向上，不收空頭。\n"
        "\n"
        "<b>查某一檔</b>\n"
    ...
    "row1": (
        ...
        "• 自動版：平日 06:30 寄黃金買點（沒檔也寫今日沒有）、優先看／周帶量（有名單才寄）；12:45 有尾盤版（現在要做的事先講）。\n"
    ...
    "screen": (
        ...
        "06:30 早報第一則是大盤狀況（美股＋台指期夜盤＋白話連動），接著寄黃金買點（買點＋還在零；沒檔也寫今日沒有）／優先看／周帶量（沒名單就跳過）；半年高／站上季線／止跌請按主選單「海選」看完整。\n"
