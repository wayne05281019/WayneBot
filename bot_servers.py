...
- 打股票代號 → 介紹圖（上半資訊、下半高低導航）＋決策卡；可滑 180 根按圖下「導航圖」
...
HELP_TOPICS = {
    "guide": (
        ...
        "\n"
        "<b>查某一檔</b>\n"
        "打股名或代號會<b>一次出兩張圖</b>：<b>介紹圖</b>（上半資訊、下半180日高低導航）→ 決策卡。要滑動對價按圖下<b>導航圖</b>或<b>K線</b>。\n"
        "\n"
        "圖下方（查完才出現，不是主選單那兩排）：\n"
        "• <b>籌碼</b>　三大法人買賣超圖\n"
        ...
        "• <b>K線</b>　開這一檔可滑動的日K（不是 TradingView）：手指對價、紫／綠箭頭跟高低卡同一套。可改 15 分／60 分，或五日／十日／月線／季線。興櫃沒這顆\n"
        "• <b>導航圖</b>　同一頁、先打開約 180 根日K，左右拖、雙指縮放；箭頭跟高低卡同一套。興櫃沒這顆\n"
        "• <b>觀察</b>　加入自選（還沒買）\n"
    ...
    "stock": (
        "<b>查股頁（圖下方按鈕）</b>\n"
        "打股名或按看這檔：一次出介紹圖、決策卡（相簿）。介紹圖下半已有 180 日高低導航；要滑動對價按圖下「導航圖」或「K線」。\n"
        "籌碼／營收／產業／K線／導航圖按<b>圖下方</b>按鈕，不是右側 ⌨️ 主選單。\n"
        "\n"
        "<b>圖下方這一排</b>\n"
        "• <b>導航圖</b>：開可滑動約 180 根日K，紫／綠箭頭跟高低卡同一套（左右拖、雙指縮放）\n"
        "• <b>籌碼</b>：三大法人買賣超圖\n"
        "• <b>營收</b>：月營收、季報毛利\n"
        "• <b>產業</b>：一張圖卡（同業中位＋本產業法人）；股名旁有公開細項小框，沒抓到不畫\n"
        "• <b>K線</b>：同一套可滑動日K；可改 15 分／60 分／五日／十日／月線／季線\n"
        "• <b>觀察</b>：加入自選（還沒買）\n"
    ...
    "pick": (
        ...
        "一次出兩張圖：介紹圖（上半資訊、下半180日高低導航）→ 決策卡。要滑動對價按圖下「導航圖」或「K線」。\n"
    ...
    def _hub_keyboard(
        ...
        *,
        ...
        nav = (
            ...
            if nav_url
            else InlineKeyboardButton("導航圖", callback_data=f"g:{c}")
        )
        actions = [
