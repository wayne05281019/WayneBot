...
def _check_lens(feature: str, lens: str, src: str, intent_src: str) -> Dict[str, Any]:
    ...
    if lens == "first_open_copy":
        ...
        if feature == "剛離零":
            return _ok("leave-zero") if "MENU_BTN_LEAVE_ZERO" in src else _bad("沒剛離零")
    ...
    if lens == "slot_noop":
        if "MENU_BTN_LEAVE_ZERO" not in src:
            return _bad("沒剛離零鈕")
        if "leave_zero_cmd" not in src:
            return _bad("沒剛離零處理")
        return _ok("leave-zero")
