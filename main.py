# -*- coding: utf-8 -*-
"""
WayneBot 核心伺服端 (Phase 1 資料庫 + Phase 2 流式清洗篩選 + Phase 3 Telegram 推播與 Render Web 極速綁定)
"""
...
# 引入 Phase 1 資料庫模組與 Phase 2 清洗篩選引擎
import wayne_db
...
# ==========================================
# 0. Render Web 端口即時監聽服務 (0.01秒極速綁定)
# ==========================================
...
def parse_twse_stocks(raw: dict, date_str: str) -> Optional[pd.DataFrame]:
    ...
    for r in data_rows:
        if len(r) < 11:
            continue
        raw_sid = str(get_col(r, 0)).strip().replace("=", "").replace('"', "")
        sid = screening_engine.normalize_ticker(raw_sid)
        if not (4 <= len(sid) <= 6 and (sid.isdigit() or sid.endswith("L") or sid.endswith("R"))):
            continue
        
        sname = str(get_col(r, 1)).strip()
        vol_shares = screening_engine.clean_number(get_col(r, 2))
        turnover_k = screening_engine.clean_number(get_col(r, 4)) / 1000.0
        open_p = screening_engine.clean_number(get_col(r, 5))
        high_p = screening_engine.clean_number(get_col(r, 6))
        low_p = screening_engine.clean_number(get_col(r, 7))
        close_p = screening_engine.clean_number(get_col(r, 8))
        sign = str(get_col(r, 9))
        diff = screening_engine.clean_number(get_col(r, 10))
...
def parse_tpex_stocks(raw: dict, date_str: str) -> Optional[pd.DataFrame]:
    ...
    for r in raw.get("aaData", []):
        if len(r) < 10:
            continue
        raw_sid = str(get_col(r, 0)).strip()
        sid = screening_engine.normalize_ticker(raw_sid)
        if not (4 <= len(sid) <= 6 and (sid.isdigit() or sid.endswith("L") or sid.endswith("R"))):
            continue
        
        sname = str(get_col(r, 1)).strip()
        close_p = screening_engine.clean_number(get_col(r, 2))
        diff = screening_engine.clean_number(get_col(r, 3))
        open_p = screening_engine.clean_number(get_col(r, 4))
        high_p = screening_engine.clean_number(get_col(r, 5))
        low_p = screening_engine.clean_number(get_col(r, 6))
        vol_shares = screening_engine.clean_number(get_col(r, 7))
        turnover_k = screening_engine.clean_number(get_col(r, 8)) / 1000.0
...
def get_or_build_history(days_to_fetch=30) -> pd.DataFrame:
    ...
    for d_str in date_list:
        ...
        if day_s:
            day_df = pd.concat(day_s, ignore_index=True)
            # Phase 2 Sanity Check 檢驗
            if screening_engine.validate_scraped_data(day_df.to_dict("records"), min_expected_count=500):
                all_stocks.append(day_df)
...
def broadcast_evening_screener():
    ...
    try:
        ...
        for _, row in top_ranked.head(10).iterrows():
            ...
            # 整合 Phase 2 獨立標的快取存檔
            screening_engine.save_filtered_data_to_cache({
                "ticker": row["stock_id"],
...
if __name__ == "__main__":
    ...
    # 3. 主執行緒直接監聽 Render 端口 (Render 檢測瞬間通過)
