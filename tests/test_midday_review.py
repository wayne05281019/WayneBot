# -*- coding: utf-8 -*-
"""12:45 尾盤：現價旁寫今早名單價與漲跌差；不寄轉 LINE 複製稿。"""
...
def test_midday_stock_line_shows_morning_price_and_diff():
    ...
    assert line.startswith("4915 致伸 現 62.1 早 60.8 ")
    assert "+1.3" in line
    assert "+2.1%" in line
...
def test_midday_html_has_no_line_share_hint():
    html = format_midday_html(
        "20260908",
        {
            "ok": ["4915 致伸 現 62.1 早 60.8 +1.3（+2.1%）"],
            "chase": [],
            "above_entry": [],
            "no_quote": [],
        },
    ...
    assert "現價旁＝今早名單價與漲跌差" in html
    assert "4915 致伸 現 62.1 早 60.8 +1.3（+2.1%）" in html
...
def test_run_midday_review_uses_pick_close_and_skips_line_share(tmp_path):
    ...
    assert "現 62.1 早 60.8" in out["html"]
    assert "+1.3" in out["html"]
...
def test_main_runner_midday_does_not_send_copy_paste():
    ...
    assert "現價旁寫今早價與漲跌差，不轉 LINE" in src
