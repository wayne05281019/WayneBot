# -*- coding: utf-8 -*-
"""捕獲發文立刻對官方日K建檔：點名檔才寫，IET 不編，盤中未收不當官方收。"""
import inspect
import os
import sqlite3
...
def test_named_pairs_skips_iet_and_quoted_bystander():
...
def test_record_events_uses_last_official_when_intraday(tmp_path):
    ...
    iet_names = [
        r[0]
        for r in conn.execute("SELECT stock_name FROM biaoke_tape").fetchall()
    ]
    assert "IET" not in iet_names
    sids = [
        r[0]
