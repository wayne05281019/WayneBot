...
def test_nine_pages_large_type_and_no_emoji(tmp_path):
    ...
    assert "剛脫離零" in blob
    assert "獲利為零" not in blob
    assert "脫離1" not in blob
...
def test_wrap_line_keeps_period_and_closing_paren():
    ...
    b = _wrap_line(
        dr,
        "一次兩張放同一則縮圖（一張圖卡）：介紹圖／高低溫度卡，接著再送大量區專圖（近窗爆大量日高低，非買訊）。點開高畫質。產業鈕出產業圖（產業鏈小框）；導航圖鈕出 180 日高低。K線開奇摩股市同一檔日K。",
        font,
        520,
        520,
    )
