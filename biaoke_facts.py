...
不要另開第二套行情庫。每天回傳融合仍寫進同一顆 wayne_market.db：
  ...
  16:30         盤後日K／法人／產業資金／月營收／季報／除權息／興櫃／
                加權／台指期／官方快照／洞燭走查；匯入晚到齊了才重算；之後把新收盤對回飆大建檔
  20:00         AI 模擬倉用收盤（不推播）
盤中 08:00–09:00 每 10 分、09:01–13:30 每 3 分、13:30–15:00 每 10 分、其餘每 1 小時抓公開文與討論串；日盤 Yahoo 15／60、夜盤期交所成交進 minute_bars。
"""
...
DAILY_FUSE_SLOTS: Tuple[Tuple[str, str], ...] = (
    ...
    ("12:45", "尾盤複核現價"),
    ("16:30", "盤後融合日K／法人／產業／基本面／加權／台指期／洞燭走查；匯入晚到齊了才重算，再對回飆大建檔"),
