    ...
    if named:
        ...
        if sid == "2383":
            ...
            bits.append(
                ...
                "9/24 他改口真突破／滾量上攻：官方最後完整柱 20260923 開 6005 高 6410 低 5930 收 6410 量 1525，"
                "收在當日高且收過 9/1 高 6095。9/24 這機還沒這列。待驗證，不是買訊。"
            )
        elif sid == "3017":
            bits.append(
                ...
                "9/24 關前整理→下星期二主升：官方近高 20260907 高 3595 收 3420 量 3303；"
                "20260923 開 3425 高 3590 低 3410 收 3470 量 2649＝回測近高、量縮、不是轉弱K。"
                "官方 20260924 開 3460 高 3600 低 3455 收 3555 量 2261＝高碰到近高、收沒有效過。"
                "下星期二（20260929）收盤有效過 3595 才算主升。9/24 高碰到≠確認。待驗證，不是保證、不是買訊。"
            )
        elif sid == "3443":
            bits.append(
            ...
            bits.append(
                ...
                "9/20：這波反彈高點一定要過 1245，附近轉弱K先走。官方 20260923 高 1140 收 1140 量 14173，還沒過 1245。"
                "9/23 他說量價整理完成。待驗證，不是買訊。"
            )
        elif sid == "6274":
            bits.append(
                "PCB 長期趨勢向上、目前底部整理。官方 20260915 台燿收 1370。"
                "10:39：台燿就是要等台光電表態，一定還會再創高，只是不是主力、無法知道何時發動。"
                "9/20：量縮不破 9/17 低則底部確認。官方 20260917 低 1275 收 1320；20260923 開 1440 高 1485 低 1420 收 1460，9/17 低沒破。"
                "9/24 14:08：到前高後一段時間整理，不太可能像聯發科直接噴。近窗高還在 8/14 高 1730 下。櫃買量欄不採。待驗證，不是買訊。"
            )
        elif sid == "6683":
            bits.append(
            ...
            bits.append(
                ...
                "9/23 主文：5 月見高後整理 4 個月有效過歷史高。官方 20260923 開 5220 高 5265 低 5140 收 5185 量 6430。"
                "台燿對這檔＝已過高後噴的對照，不是把波浪套個股。"
            )
        elif sid == "2455":
            bits.append(
                "9/23：全新比聯亞更穩健、這階段是 InP 風向球。"
                "9/24 08:38：全新沒這麼快整理完成；回測支撐量縮買點要尾盤才知道。"
                "官方 20260917 高 566 量 28618；20260923 開 541 高 552 低 535 收 552 量 3310。"
                "待驗證，不是買訊。"
            )
    if want_trend:
        try:
...
def _hold(brief: Dict[str, Any], ask: str, *, named: bool, db_path: str = "", uid: str = "") -> Dict[str, Any]:
    ...
    if sid == "2383":
        ...
        bits.append(
            ...
            "假跌破過幾天。止漲整理K 才調節。"
            "9/24 真突破：官方 20260923 收 6410 當日高、過 9/1 高 6095。待驗證，不是買訊。"
        )
    elif sid == "3017":
        bits.append(
            "不看月線。因健策轉折K盤中調 1/2。"
            ...
            "模糊≠轉折K，不准套2天漲1000續抱。"
            "9/24 關前：官方 20260907 高 3595；20260923 高 3590 收 3470 量 2649 回測近高量縮，不是轉弱K。"
            "下星期二主升待驗證有效過 3595，不是保證。"
        )
    elif sid == "3443":
        ...
        bits.append(
            "等台光電表態才知何時創高；自己還在底部。"
            "9/24：到前高後整理，不像聯發科噴。官方 20260923 高 1485 收 1460，9/17 低 1275 沒破。"
        )
...
def _five_cross(steps: List[Dict[str, Any]], sid: str, name: str = "") -> str:
    ...
    if (nest_build or nest_cover) and own_keyk:
        ...
        if sid == "3653":
            bits.append(
                "9/24真突破：官方20260923收6410當日高、過9/1高6095。待驗證，不是買訊。"
            )
        ...
        if sid == "3017":
            bits.append(
                "9/24關前量價：官方20260907高3595；20260923高3590收3470量2649回測近高量縮、不是轉弱K。"
                "官方20260924高3600收3555＝高碰到近高、收沒有效過。"
                "下星期二（20260929）收盤有效過3595才算主升。9/24高碰到≠確認。待驗證，不是保證、不是買訊。"
            )
    elif wash:
        bits.append(
            prefix
