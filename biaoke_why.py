...
點位對官方加權／台指期日 K；夜盤 15 分沒有就不數段。日盤 15 分有庫只報高低，不發明段。不是買訊。
"""
...
_TOPICS: List[Dict[str, Any]] = [
    {
        ...
        "a": (
            ...
            "同則「15 分細微波走 5 段、下降軌已破壞」是他夜盤 15 分圖上數的；夜盤 15 分 Yahoo 沒有，不數段、不假裝畫過那條軌。"
        ),
    },
    {
        "id": "47578",
    ...
    },
    {
        ...
        "a": (
            ...
            "9/11 17:49 是他夜盤 15 分圖上數完 5 段。夜盤 15 分 Yahoo 沒有，不能假裝數過這則的段。"
    ...
    },
    {
        ...
        "a": (
            ...
            "夜盤 15 分 Yahoo 沒有，不能假裝畫過圖上那條線。60 分下降壓他公開過從 48218 連下來（7/23）。"
            "有畫才知道被破壞——沒附圖、沒夜盤 15 分就只准講方法，不准發明連哪兩根。"
        ),
    },
    {
        "id": "degree2",
    ...
    },
    {
        ...
        "a": (
            ...
            "某金融商品沒點名，不准寫死。夜盤 15 分沒有仍不數段。不是買訊。"
        ),
    },
]
    ...
    if _FIFTEEN.search(text):
        ...
        if db_path:
            try:
                ...
                cover = minute_day_cover("TWII", "15", date, db_path)
                bits.append(
                    minute_cover_note(cover, night=bool(re.search(r"夜盤", text)))
                )
            except Exception:
                bits.append("庫沒 15 分，不數這則的段。")
        else:
            if re.search(r"夜盤", text):
                bits.append("夜盤 15 分 Yahoo 沒有，不數這則的段。")
            else:
                bits.append("庫沒 15 分，不數這則的段。")
