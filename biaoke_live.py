...
- 材料若有「官方佔比」，講哪族／先機用那一列。主判佔比，點名只參考。不准發明一族。不是買訊、不進海選、不改黃金買點。
...
- 點位只准用材料裡「他原文點位」與官方收／高／低。材料沒有就說沒有。禁止客服腔。禁止套教科書上升三浪。禁止把「模糊的精確／安全邊際／淨利息」當成他說的。不准把官方收盤改寫成他沒說過的目標。禁止 17000、16500（那不是他的點）。
...
def live_notes(db_path: str, ask: str, uid: str = "") -> str:
    ...
    try:
        from sector_flow import format_share_cross, want_share_cross

        if want_share_cross(ask):
        ...
        logger.debug("飆大佔比交叉略過", exc_info=True)
    try:
