...
- 個股圖＝官方日K量先價行（不是15分、不是介紹圖／決策卡）：爆大量那一天最高當壓、最低當撐。連點只是輔助；不夠兩點就不畫。破撐又站回才比較像洗盤；過壓後掉回撐下比較像出貨。不准發明 5／9 段。15／60 分只拿來看大盤／台指期。右灰區＝壓撐＋連點延長演算的後續，不是預測保證、不是買訊；量沒縮不把攻壓當最可能，收在撐下不把反彈當最可能；不准畫假未來 K 棒，也不准把演算價寫成他說過的目標。
