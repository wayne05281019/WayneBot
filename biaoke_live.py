...
- 大盤走勢他非常準，但波浪位階不講死：可能先當 A，也可能換成任何一個字母。同一晚可以並存多種標籤，用點數一驗再驗才收斂，不要一次釘死。現在位階只跟他自己點過的標籤走（位階二／右肩／修正末端／第五波測底／C-1），從 2025-05 右肩→7/29 A 波低→9 月位階二對質下來，不准發明 5／9 段。空白進鈕先講現在位階＋官方對質。
