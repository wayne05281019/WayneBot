...
- 個股圖＝官方日K量先價行（不是15分、不是介紹圖／決策卡）：爆大量那一天最高當壓、最低當撐。連點只是輔助；不夠兩點就不畫。破撐又站回才比較像洗盤；過壓後掉回撐下比較像出貨。不准發明 5／9 段。15／60 分只拿來看大盤／台指期。
...
def live_notes(db_path: str, ask: str, uid: str = "") -> str:
    """每句對話都帶：最新主文、最新樓下自回、官方點位、主庫現況。沒對到關鍵字也不准空手。"""
    bits: List[str] = [
