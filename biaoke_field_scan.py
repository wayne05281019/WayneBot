...
量起來若是轉弱／還要再跌的量價（爆量長上影／爆量收在當日下半）不准當發動。個股不數 5／9。盤中未收不當官方收。
...
def rotation_screen_block(db_path: str, *, spoken: Optional[str] = None) -> str:
    """海選末段只寫這型是先機還是已當第一。方法論、近100日板、門檻不進話筒。"""
    from tg_layout import pack_phone_bits
    ...
    now_rows = pack_phone_bits(_esc(field), _esc(tag) if tag else "")
    if sign == "pre" and data.get("pre_ok") and data.get("pre_vs20") is not None:
        now_rows.append(_esc(f"次級距20高 {float(data.get('pre_vs20') or 0):+.1f}%。"))
    ...
    if recs and sign == "pre":
        ...
        now_rows.extend(_phone_note_lines("進場只認剛離零。"))
    return _blk(*now_rows)
